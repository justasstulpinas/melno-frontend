      ...
      {!loading && templates.length > 0 && (
        ...
        <div className="flex flex-col gap-4">
          ...
          <button
            type="button"
            onClick={() => { setActionError(""); fileInputRef.current?.click(); }}
            className={`flex items-center justify-center gap-3 border border-dashed rounded-xl min-h-[120px] transition-all duration-150 ${
              dragOver ? "border-white/40 bg-white/[0.02]" : "border-zinc-800 hover:border-zinc-600"
            }`}
          >
            <svg className={`w-5 h-5 transition-colors ${dragOver ? "text-white/50" : "text-zinc-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span className={`text-sm transition-colors ${dragOver ? "text-white/60" : "text-zinc-600"}`}>
              {uploading ? "Konvertuojama…" : dragOver ? "Paleiskite norėdami įkelti" : "Įkelti naują .docx šabloną"}
            </span>
          </button>
        </div>
        </>
